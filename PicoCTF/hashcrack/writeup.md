...
Hashing is not cryptography, and this is a cryptography challenge. When I think of hashing, I immediately think of common hashes which are used for file integrity. 

More specifically, from: https://www.techtarget.com/searchdatamanagement/definition/hashing
__"A hash function generates new values according to a mathematical hashing algorithm, known as a hash value or simply a hash. To prevent the conversion of a hash back into the original key, a good hash always uses a one-way hashing algorithm."__. The fact that a hash is uni-directional means that it is does not provide a method for decrypting hashed data. In order to retrieve the values that were used to generate the hash, we would need to hash a large set of values and then make note of what each value is so that we can compare it with the hash in front of us.

Within Kali Linux, we have some hash cracking tools available to us such has hashcat: https://hashcat.net/hashcat/


<details>
<summary> Hash cat --help</summary>
